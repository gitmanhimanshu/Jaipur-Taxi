...
const AdminTourBookings = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(booking => 
        ...
        (booking.tourDetails?.name && booking.tourDetails.name.toLowerCase().includes(searchQuery.toLowerCase()))
      )
    }
  ...
  const fetchTourBookings = async () => {
    try {
      setIsLoading(true)
      const response = await bookingsAPI.getAdminTourBookings()
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage bookings for your tours</p>
            </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-3">
                        <h3 className="text-lg font-semibold text-gray-900">
                          {booking.tourDetails?.name || 'Tour Booking'}
                        </h3>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-900">{selectedBooking.tourDetails?.name || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Type</p>
                        <p className="text-gray-900">{selectedBooking.tourDetails?.type || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Duration</p>
                        <p className="text-gray-900">{selectedBooking.tourDetails?.duration || 'N/A'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Price</p>
                        <p className="text-gray-900">{selectedBooking.tourDetails?.price || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </div>
